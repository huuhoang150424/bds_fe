...
import { useSearchNew } from "../hook/use-search-new";

export default function ListNewBySearch ({keyWord}: { keyWord: string })
{
  const { data, isLoading } = useSearchNew(keyWord);
  ...
  return (
    <div className="mb-[30px] mt-[-40px]">
      { isLoading ? (
        ...
        <div>
          { data?.rows.map( ( news: any,index:number ) => (

...
export default function SearchBar({ allProvinces, onSearch, showMap, setShowMap }: SearchBarProps) {
  ...
  const filteredProvinces = allProvinces
    .filter(
      (province) =>
        province.toLowerCase().includes(debouncedSearchInput.toLowerCase()) && !selectedProvinces.includes(province),
    )
    .slice(0, 10);
    ...
    <div className='flex flex-col md:flex-row md:items-center mb-[20px] gap-4'>
      <div className='relative w-full'>
        <div className='flex items-center border rounded-lg p-2 bg-white'>
          ...
          <Button className='bg-[#E03C31] hover:bg-[#FF837A] text-white' onClick={handleSearch}>
            Tìm kiếm
          </Button>
        </div>

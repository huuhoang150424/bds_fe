...
import { MapPin, Bed, Bath, Maximize, Phone, Heart } from 'lucide-react';
  ...
  return (
    <Card className='overflow-hidden hover:shadow-xl transition-shadow border border-gray-200 rounded-lg'>
      <div className='relative'>
        {post.priority > 0 && (
          <Badge className='absolute top-3 left-3 z-1 bg-red-600 text-white font-bold px-4 py-1'>VIP KIM CƯƠNG</Badge>
        )}
      ...
      <CardContent className='p-4'>
        <div className='space-y-2'>
          <h3 className='text-lg font-semibold hover:text-[#E03C31] cursor-pointer transition-colors line-clamp-2'>
            {post.title}
          </h3>
          ...
          <div className='flex flex-wrap items-center gap-x-6 gap-y-2'>
            <div className='flex items-center gap-2'>
              <span className='text-[#E03C31] font-bold text-base'>
                {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(post.price)}
              </span>
            </div>
            <div className='flex items-center gap-2 text-gray-700'>
              <Maximize className='h-5 w-5' />
              <span className='font-medium'>{post.squareMeters} m²</span>
            </div>
          ...
          <div className='text-gray-700 line-clamp-2 text-sm pt-1'>
            {post.description ||
              `The Origami. Giải pháp tích sản thông minh, ai cũng có thể sở hữu. Liên hệ: ${post.user?.phone || '0703 888 ***'} (${post.user?.fullname || 'Ms Hanh'}). Căn ${post.bedroom}PN loại căn diện tích lớn ${post.squareMeters}m² giá chỉ ${(post.price / 1000000000).toFixed(2)} tỷ.`}
          </div>
        </div>
        <div className='mt-3 pt-3 border-t border-gray-200 flex items-center justify-between'>
          <div className='flex items-center gap-2'>
            ...
            <div>
              <p className='text-sm font-semibold'>{post.user?.fullname || 'Hanh Pham'}</p>
              <p className='text-xs text-gray-500'>Đăng hôm nay</p>
            </div>
          </div>
          <div className='flex items-center gap-2'>
            <Button
              variant='default'
              className='flex items-center gap-2 bg-[#E03C31] hover:bg-[#d02e23] text-white'
              size='sm'
            >

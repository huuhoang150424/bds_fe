...
import TwoFactorDialog from './two-2fa';
...
function LoginScreen() {
  ...
  const [twoFactorData, setTwoFactorData] = useState<{ open: boolean; userId: string }>({
    open: false,
    userId: '',
  });
  ...
  }, [message, error, success, closeModal]);

  function onSubmit(values: z.infer<typeof formSchemaLogin>) {
    dispatch(loginAuth(values)).then((result) => {
      console.log(result)
  ...
  return (
    <div className="w-[45%] p-8">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="bg-[#fff] rounded-[10px] mb-[15px] flex justify-center border-[1px]">
              <LoginGoogle setTwoFactorData={setTwoFactorData} />
            </div>
      ...
      <TwoFactorDialog open={twoFactorData.open} userId={twoFactorData.userId} />
    </div>
  );
}

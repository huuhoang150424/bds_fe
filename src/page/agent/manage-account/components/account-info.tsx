      ...
      <CardContent className="pt-4 grid gap-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1.5 p-3 rounded-lg bg-gray-50 dark:bg-gray-900/50 border border-gray-100 dark:border-gray-800">
            ...
            <div className="font-mono text-xs bg-gray-100 dark:bg-gray-800 p-2 rounded overflow-x-auto">
              {user.id || 'Chưa xác định'}
            </div>
          </div>
          ...
          <div className="space-y-1.5 p-3 rounded-lg bg-gray-50 dark:bg-gray-900/50 border border-gray-100 dark:border-gray-800">
            ...
            <div className="font-medium text-sm flex items-center">
              <span className="text-amber-500 font-bold mr-1.5">{user.score || 0}</span>
              <div className="flex">
                {[...Array(5)].map((_, i) => (
                  <svg
                    key={i}
                    className={`h-3 w-3 ${i < Math.floor((user.score || 0) / 20) ? 'text-amber-400' : 'text-gray-300 dark:text-gray-600'}`}
                    viewBox="0 0 20 20"
                    fill="currentColor"
                  >
          ...
          <div className="space-y-1.5 p-3 rounded-lg bg-gray-50 dark:bg-gray-900/50 border border-gray-100 dark:border-gray-800">
            ...
            <div className="font-medium text-sm">
              {user.lastActive
                ? format(new Date(user.lastActive), 'HH:mm - dd/MM/yyyy', { locale: vi })
                : 'Chưa cập nhật'}
            </div>
          </div>
        </div>

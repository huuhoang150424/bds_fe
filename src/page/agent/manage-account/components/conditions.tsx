...
import { selectUser, updateProfessionalStatus } from '@/redux/authReducer';
import { useRegisterProfessionalAgent } from '../hooks/use-register-professional';
...
export default function Conditions({
  ...
  const [missingConditions, setMissingConditions] = useState<string[]>([]); // New state for missing conditions
  const dispatch = useDispatch();
  const user = useSelector(selectUser);
  const { mutate: registerProfessional, isPending } = useRegisterProfessionalAgent();

  const tabs = [
    { id: 'account', label: 'Tài khoản', icon: <Shield className='h-4 w-4' />, color: 'from-red-300 to-red-400' },
    { id: 'activity', label: 'Hoạt động', icon: <Activity className='h-4 w-4' />, color: 'from-rose-300 to-rose-400' },
    { id: 'financial', label: 'Tài chính', icon: <CreditCard className='h-4 w-4' />, color: 'from-pink-300 to-pink-400' },
  ...
  const conditions: any = {
    account: [
      ...
      { id: 4, name: 'Hoạt động ít nhất 1 năm' },
    ],
    activity: [
    ...
    financial: [
      ...
      { id: 10, name: 'Từng nạp 1 triệu đồng' },
      { id: 11, name: 'Từng mua gói thành viên' },
    ],
    profile: [
  ...
  const onRegister = () => {
    ...
    registerProfessional(undefined, {
      onSuccess: (response) => {
        console.log(response.data.data.missingConditions);
        if (response.data.success) {
          dispatch(updateProfessionalStatus(true));
          toast({
            title: 'Thành công',
            description: 'Đăng ký môi giới chuyên nghiệp thành công!',
          });
          onOpenChange(false);
        } else {
          // Set missing conditions to state instead of showing toast
          setMissingConditions(response.data.data.missingConditions || []);
        }
      },
      onError: (error: any) => {
        toast({
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='max-w-md p-0 overflow-hidden bg-white rounded-xl shadow-2xl border-0'>
        ...
        <div className='p-4 max-h-[60vh] overflow-y-auto bg-gradient-to-br from-gray-50 to-white'>
          ...
          {/* Render missing conditions if they exist */}
          {missingConditions.length > 0 && (
            ...
            >
              <p className='text-sm font-medium text-red-800 mb-2'>Không đủ điều kiện để đăng ký:</p>
        ...
        <div className='p-4 bg-gray-50 border-t'>
          <div className='relative group'>
            ...
            <Button
              onClick={onRegister}
              disabled={isPending}
              className='relative w-full bg-gradient-to-r from-red-400 via-red-500 to-rose-400 hover:from-red-500 hover:via-red-600 hover:to-rose-500 text-white text-sm rounded-md py-5 disabled:opacity-50'
            >

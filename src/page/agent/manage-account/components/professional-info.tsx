...
import { BriefcaseIcon, BookOpenIcon, AwardIcon,Medal, Check } from 'lucide-react';
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
          <div className='space-y-1.5 p-3 rounded-lg bg-gray-50 dark:bg-gray-900/50 border border-gray-100 dark:border-gray-800'>
            ...
            <div className='font-medium text-sm flex items-center'>
              <span className='font-bold mr-1'>{user.experienceYears}</span>
              <span>năm</span>
            </div>
            <div className='w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 mt-1.5'>
              <div
                className='bg-blue-500 h-1.5 rounded-full'
                style={{ width: `${Math.min(Number.parseInt(user.experienceYears) * 10, 100)}%` }}
              ></div>
            </div>
          </div>
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 gap-2'>
                {user.certificates.split(',').map((cert: string, index: number) => (
                  ...
                  >
                    ...
                    <span className='font-medium text-xs text-gray-800 dark:text-gray-200'>{cert.trim()}</span>
                  </div>
                ))}
              </div>
            ) : (
